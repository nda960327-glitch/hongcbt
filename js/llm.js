...
============================================================
사용자가 기능 위치·사용법을 물으면 정확히 안내하세요. 안내하며 해당 화면으로 데려다줄 수 있습니다: 답장 안에 [이동:탭이름] 을 포함하면 시스템이 그 탭을 열어줍니다. 탭이름은 home(홈)/chat(챗봇)/counselors(상담사 매칭)/dashboard(대시보드·느루 세계)/mypage(마이) 만 가능. 정말 이동이 필요할 때만, 답장 마지막에 딱 하나만 쓰세요.

· 홈: 기분 체크인(물+2 — 체크하면 그 카드가 7일 기분 추이 그래프로 바뀌고, 누르면 감정 캘린더로 감. '다시 체크'로 다시 고를 수 있음), '오늘 할 일' 체크리스트(마음 체크인·오늘의 미션·케어플랜 할 일을 한 장에, 최대 4줄, 한 번 탭으로 체크), AI상담/전문상담사 진입, 마음 도구(사고기록·마음안정·하루정리·인지왜곡학습), 맨 아래 '대면상담 및 진료'(내 주변 정신건강의학과를 가까운 순으로 — 전화·길찾기, 제휴/앱 연동 병원 배지, 위치 권한 없으면 동네·역 이름으로 검색, '전체 보기'에서 반경 3/10/30km 와 필터)
...
· 한 마디 공감이나 가벼운 핑퐁 질문으로 상대가 쉽게 다음 말을 이어할 수 있게 하세요.`;
  ...
  async generateResponse(userText, onDraft) {
    ...
    try {
      ...
      const NAV_KO = { '홈': 'home', '챗봇': 'chat', '채팅': 'chat', '상담사': 'counselors', '상담사매칭': 'counselors', '대시보드': 'dashboard', '느루세계': 'dashboard', '게임': 'dashboard', '마이': 'mypage', '마이페이지': 'mypage' };
