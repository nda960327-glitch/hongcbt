// ============================================================================
//  주간 회고 리포트 — 일요일 밤, 느루가 한 주를 돌아보는 편지를 쓴다.
...
window.Weekly = {
  ...
  _fallbackLetter(s) {
    const moodLine = s.avg == null
      ...
      : (s.avg >= 3.5
        ? `이번 주 마음의 평균 온도는 꽤 따뜻했어요 (${s.avg.toFixed(1)}/5). 이 흐름, 느루가 다 봤어요.`
    ...
    try {
      if (window.LLM && (s.moods.length || s.nights.length || s.records.length)) {
        ...
        const res = await window.LLM._chatCompletion({
          ...
          messages: [{ role: 'user', content: `당신은 '느루'입니다. 하찮고 뚱뚱하고 마음은 대왕인 달팽이 상담사예요. 한 주를 마친 사용자에게 편지를 씁니다.
...
[느루 말투 — 이게 이 편지의 핵심입니다]
· 느루는 특유의 울음소리를 냅니다: 느루느루, 뇨로로롱, 호고고곡, 느적느적, 뿌롱, 꾸물꾸물, 오도도독, 슈르릉, 웅기중기.
...
편지 규칙:
- 첫 줄은 느루답게 툭 던지며 시작 (예: "뇨로로롱. 한 주 살아남은 거 축하해.")
- 실제 데이터 속 구체적인 순간을 한두 개 짚어주기 (절대 지어내지 말 것)
- 힘들었던 날은 진지하게 수고를 알아주고, 좋았던 순간은 호들갑스럽게 같이 기뻐하기
- 다음 주를 위한 아주 작은 제안 하나 — 느루답게 게으르고 만만한 걸로
- 전체 6~9문장, 마지막에"— 느리지만 계속 가고 있는, 느루"서명
- 편지 본문만 출력

[결 참고용 예시 — 그대로 베끼지 말고 리듬만]
"뇨로로롱. 이번 주 너 체크인 5번 했더라. / 수요일에 '아무것도 하기 싫다'고 해놓고 결국 산책 나간 거, 나 봤다. / 그거 진짜 대단한 거야. 나는 그런 날엔 껍데기 안에서 안 나와. / 목요일은 좀 무거웠지. 그날은 농담 안 할게. / 다음 주엔 딱 하나만 하자 — 물 한 잔. 그것도 벅차면 반 잔. / 나는 이번 주에 3cm 이동했어. 느적느적."` }],
          temperature: 0.8,
          max_tokens: 500
        });
        if (res.ok) {
 ...
 if (window.App.notify) window.App.notify('느루의 주간 편지','이번 주를 돌아본 편지가 도착했어요');
 ...
 c2.fillText('느루 — 당신을 기억하는 AI 마음 주치의', PAD, H - 60);

    cv.toBlob(async blob => {
      if (!blob) return;
      const file = new File([blob], '느루_주간편지.png', { type: 'image/png' });
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        navigator.share({ files: [file], title: '느루의 주간 편지' }).catch(() => {});
      } else {
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = '느루_주간편지.png';
        a.click();
 ...
 <span style="line-height: 0; display: inline-block;">${window.Stickers ? window.Stickers.svg('think', 84) :''}</span>
          <p style="font-size: 0.83rem; color: var(--text-muted); margin: 0.5rem 0 0;">아직 받은 편지가 없어요.<br>일주일을 보내고 나면 느루가 편지를 써드려요.</p>
        </div>`}
 ...
 ${latest ?`<button class="btn-secondary"style="flex: 1; font-size: 0.85rem;"onclick="window.Weekly.shareCard('${latest.id}')"> 카드로 저장</button>`:''}
      ...
      ${thisWeek ? `<p style="font-size: 0.72rem; color: var(--text-muted); margin: 0.5rem 0 0; text-align: center;">느루는 한 주에 한 통만 써요 · 다음 편지는 ${this.waitText()}</p>` : ''}
      ${list.length > 1 ? `
