...
function viewSettings() {
  ...
  return `
    ...
    <p class="muted" style="margin-top: 1rem;">
      마인드 인사이드 운영자 콘솔 · 데스크톱 우선 · 다크 모드 없음<br>
...
function viewClinics() {
  ...
  const form = `
    <div class="card" id="cl-form">
      ...
      <div class="row wrap" style="gap: 0.5rem; margin-bottom: 0.6rem;">
        <input id="cl-q" type="text" placeholder="병원 이름이나 주소로 찾기 (전국 수집된 자료에서 · 예: 마음 강남구)" style="flex: 1 1 260px;" autocomplete="off">
  ...
  return `
    ...
    ${d.partners.length ? d.partners.map(card).join('')
      : '<div class="card"><div class="empty"><b>제휴 병원이 아직 없어요</b>＋ 제휴 등록 → 이름으로 찾아 선택하면 주소·좌표가 채워집니다.</div></div>'}`;
}
async function clinicAct(act, el, id) {
  ...
  if (act === 'cl-save') {
    ...
    if (!body.lat || !body.lng) { alertBox('좌표가 필요해요', '위에서 찾아 선택하면 자동으로 채워집니다.'); return; }
